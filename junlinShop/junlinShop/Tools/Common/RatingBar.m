...
-(void)setImageDeselected:(NSString *)deselectedName halfSelected:(NSString *)halfSelectedName fullSelected:(NSString *)fullSelectedName andDelegate:(id<RatingBarDelegate>)delegate{
    ...
    //星星图片之间的间距
    CGFloat space = (CGFloat)(frame.size.width - width*5)/6;
    
    CGFloat startX = space;
    CGFloat y = (frame.size.height - height) / 2;
    [_s1 setFrame:CGRectMake(startX, y, width, height)];
    startX += width + space;
    [_s2 setFrame:CGRectMake(startX, y, width, height)];
    startX += width + space;
    [_s3 setFrame:CGRectMake(startX, y, width, height)];
    startX += width + space;
    [_s4 setFrame:CGRectMake(startX, y, width, height)];
    startX += width + space;
    [_s5 setFrame:CGRectMake(startX, y, width, height)];
